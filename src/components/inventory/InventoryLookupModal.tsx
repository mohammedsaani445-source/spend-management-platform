...
import { Search, Camera, Package, MapPin, Info, RefreshCcw, AlertTriangle } from "lucide-react";
            ...
            <div className="modal-backdrop" style={{ animation: "fadeIn 0.2s ease-out" }}>
                <div className="modal" style={{ maxWidth: '500px', width: '95%', maxHeight: '90vh', display: 'flex', flexDirection: 'column', animation: "slideUp 0.3s ease-out", padding: 0, overflow: 'hidden' }}>
                        ...
                        {isScanning && (
                            <div style={{ marginBottom: '1.5rem' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                                    <span style={{ fontSize: '0.8rem', fontWeight: 700, color: '#334155' }}>Calling Camera Viewfinder...</span>
                                    <button onClick={() => setIsScanning(false)} style={{ fontSize: '0.75rem', color: 'var(--brand)', fontWeight: 700, background: 'none', border: 'none', cursor: 'pointer' }}>Cancel Scan</button>
                                </div>
                                <BarcodeScanner
                                    onScan={handleScan}
                            ...
                            <div style={{
                                ...
                                {scanError ? (
                                    <>
                                        ...
                                        <button onClick={() => setScanError(null)} style={{ marginTop: '1rem', color: 'var(--brand)', fontWeight: 700, background: 'none', border: 'none', cursor: 'pointer' }}>Try Again</button>
                                    </>
                                ) : (
                                    <>
                    ...
                    <div className="modal-footer" style={{ padding: "1.25rem 1.5rem", borderTop: "1px solid var(--border)", background: "var(--surface-hover)", flexShrink: 0 }}>
                        <button onClick={onClose} className="btn btn-primary" style={{ width: '100%', height: '44px' }}>Done</button>
                    </div>
                </div>
            </div>
        </Portal>
