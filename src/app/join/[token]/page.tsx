    ...
    if (status === "error") {
        return (
            <div className="min-h-screen bg-black flex items-center justify-center p-6 font-sans">
                <div className="max-w-md w-full bg-[#0A0A0A] border border-white/10 rounded-xl p-10 text-center shadow-2xl relative overflow-hidden">
                    <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-transparent via-red-500/50 to-transparent"></div>
                    
                    <div className="w-20 h-20 rounded-xl bg-red-500/10 border border-red-500/20 flex items-center justify-center mx-auto mb-8">
                        <ShieldAlert size={40} className="text-red-500" />
                    </div>
                    
                    <h1 className="text-2xl font-black text-white uppercase tracking-tight mb-4">Invalid or Expired Link</h1>
                    
                    <p className="text-white/40 text-sm leading-relaxed mb-10 px-4">
                        This invite link has either expired, was already used, or is no longer valid.
                    </p>
                    
                    <div className="space-y-4">
                        <Link 
                            href="/login"
                            className="flex items-center justify-center gap-3 bg-white text-black py-4 rounded-lg font-black uppercase tracking-widest text-xs hover:bg-orange-500 hover:text-white transition-all shadow-xl shadow-white/5 w-full"
                        >
                            Back to Login <Home size={16} />
                        </Link>
                    </div>
                </div>
            </div>
        ...
        <div className="min-h-screen bg-black flex items-center justify-center p-6">
            <div className="text-center animate-in fade-in duration-700">
                <div className="relative mb-12">
                   ...
                   <div className="w-24 h-24 rounded-xl bg-white/[0.03] border border-white/10 flex items-center justify-center mx-auto relative z-10">
                ...
                <h1 className="text-3xl font-black text-white uppercase tracking-[0.2em] mb-4">
                    {status === 'valid' ? 'Verified' : 'Authenticating...'}
                </h1>
                
                <p className="text-white/30 font-medium italic tracking-widest uppercase text-[10px]">
                    {status === 'valid' ? 'Redirecting to setup...' : 'Validating invite link...'}
                </p>
                ...
                {status === 'valid' && (
                    <div className="mt-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
                        <div className="flex items-center justify-center gap-3 text-green-500 font-black text-xs uppercase tracking-widest">
                            Invite Verified <ArrowRight size={16} className="animate-pulse" />
                        </div>
                    </div>
                )}
            </div>
